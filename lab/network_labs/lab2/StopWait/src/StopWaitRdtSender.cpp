...
bool StopWaitRdtSender::send(const Message &message)
{
	...
	memcpy(this->packetWaitingAck.payload, message.data,
	       sizeof(message.data));
	...
	pns->startTimer(SENDER, Configuration::TIME_OUT,
			this->packetWaitingAck.seqnum); //启动发送方定时器
...
void StopWaitRdtSender::receive(const Packet &ackPkt)
{
	if (this->waitingState ==
	    true) { //如果发送方处于等待ack的状态，作如下处理；否则什么都不做
		//检查校验和是否正确
		...
		if (checkSum == ackPkt.checksum &&
			...
			this->expectSequenceNumberSend =
				1 -
				this->expectSequenceNumberSend; //下一个发送序号在0-1之间切换
			this->waitingState = false;
			pUtils->printPacket("发送方正确收到确认", ackPkt);
			pns->stopTimer(
				SENDER,
				this->packetWaitingAck.seqnum); //关闭定时器
		} else {
			pUtils->printPacket(
			...
			pns->stopTimer(
				SENDER,
				this->packetWaitingAck.seqnum); //首先关闭定时器
			pns->startTimer(SENDER, Configuration::TIME_OUT,
					this->packetWaitingAck
						.seqnum); //重新启动发送方定时器
			pns->sendToNetworkLayer(
				RECEIVER,
				this->packetWaitingAck); //重新发送数据包
		}
	}
}
...
{
	//唯一一个定时器,无需考虑seqNum
	pUtils->printPacket("发送方定时器时间到，重发上次发送的报文",
			    this->packetWaitingAck);
