...
int register_rfs()
{
	struct file_system_type *fs_type =
		(struct file_system_type *)alloc_page();
...
int rfs_format_dev(struct device *dev)
{
	...
	super->size = 1 + RFS_MAX_INODE_BLKNUM + 1 +
		      RFS_MAX_INODE_BLKNUM * RFS_DIRECT_BLKNUM;
	...
	for (int inode_block = 0; inode_block < RFS_MAX_INODE_BLKNUM;
	     ++inode_block) {
...
{
	int n_block =
		n_inode / (RFS_BLKSIZE / RFS_INODESIZE) + RFS_BLK_OFFSET_INODE;
...
ssize_t rfs_read(struct vinode *f_inode, char *r_buf, ssize_t len, int *offset)
{
	...
	if (readtimes >= 0) {
		...
		while (readtimes != 0) {
			rfs_r1block(rdev, f_inode->addrs[block_offset]);
			memcpy(buffer + buf_offset, rdev->iobuffer,
			       RFS_BLKSIZE);
		  ...
		  int *offset)
{
	...
	if (align != 0) {
		rfs_r1block(rdev, f_inode->addrs[block_offset]);
		int first_block_len =
			(writetimes == 0 ? len : RFS_BLKSIZE - align);
	...
	if (writetimes >= 0) {
		...
		while (writetimes != 0) {
			if (block_offset ==
			    f_inode->blocks) { // need to create new block
				// allocate a free block for the file
				f_inode->addrs[block_offset] =
					rfs_alloc_block(f_inode->sb);
				f_inode->blocks++;
			}
...
struct vinode *rfs_lookup(struct vinode *parent, struct dentry *sub_dentry)
{
	...
	for (int i = 0; i < total_direntrys; ++i) {
		if (i % one_block_direntrys ==
		    0) { // read in the disk block at boundary
			rfs_r1block(rdev,
				    parent->addrs[i / one_block_direntrys]);
...
struct vinode *rfs_create(struct vinode *parent, struct dentry *sub_dentry)
{
	...
	int free_inum = 0;
	for (int i = 0;
	     i < (RFS_BLKSIZE / RFS_INODESIZE * RFS_MAX_INODE_BLKNUM); ++i) {
	      ...
	      int *offset)
{
	...
	case LSEEK_SET:
		...
		*offset = new_offset;
		break;
	case LSEEK_CUR:
		if (*offset + new_offset < 0 ||
		    *offset + new_offset > file_size) {
			sprint("rfs_lseek: invalid offset!\n");
			return -1;
		}
		*offset += new_offset;
	     ...
	     struct vinode *link_node)
{
	...
	// change vinode of linkfile
	link_node->nlinks++;

	// change rfs dentry tree
	rfs_add_direntry(parent, name_new_link_file, link_node->inum);

	// update dinode according to vinode
	       ...
	       struct vinode *unlink_vinode)
{
	...
	for (delete_index = 0; delete_index < total_direntrys; ++delete_index) {
		...
		if (delete_index % one_block_direntrys == 0) {
			rfs_r1block(rdev, parent->addrs[delete_index /
							one_block_direntrys]);
			p_direntry = (struct rfs_direntry *)rdev->iobuffer;
		}
		if (strcmp(p_direntry->name, sub_dentry->name) == 0) { // found
	...
	memmove(rdev->iobuffer + offset * sizeof(struct rfs_direntry),
		...
		(one_block_direntrys - offset - 1) *
			sizeof(struct rfs_direntry));

	struct rfs_direntry *previous_block =
		(struct rfs_direntry *)alloc_page();
	...
	for (int i = delete_block_index + 1; i < parent->blocks; i++) {
		...
		struct rfs_direntry *this_block =
			(struct rfs_direntry *)alloc_page();
...
int rfs_hook_opendir(struct vinode *dir_vinode, struct dentry *dentry)
{
	...
	void *pdire = NULL;
	void *previous = NULL;
	struct rfs_device *rdev =
		rfs_device_list[dir_vinode->sb->s_dev->dev_id];
