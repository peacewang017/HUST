...
struct vinode_ops {
	...
	ssize_t (*viop_write)(struct vinode *node, const char *buf, ssize_t len,
			      int *offset);
	struct vinode *(*viop_create)(struct vinode *parent,
				      struct dentry *sub_dentry);
	int (*viop_lseek)(struct vinode *node, ssize_t new_off, int whence,
			  int *off);
	...
	int (*viop_readdir)(struct vinode *dir_vinode, struct dir *dir,
			    int *offset);
	struct vinode *(*viop_mkdir)(struct vinode *parent,
				     struct dentry *sub_dentry);
...
#define viop_read(node, buf, len, offset) \
	(node->i_ops->viop_read(node, buf, len, offset))
#define viop_write(node, buf, len, offset) \
	(node->i_ops->viop_write(node, buf, len, offset))
#define viop_create(node, name) (node->i_ops->viop_create(node, name))
#define viop_lseek(node, new_off, whence, off) \
	(node->i_ops->viop_lseek(node, new_off, whence, off))
#define viop_disk_stat(node, istat) (node->i_ops->viop_disk_stat(node, istat))
#define viop_link(node, name, link_node) \
	(node->i_ops->viop_link(node, name, link_node))
#define viop_unlink(node, name, unlink_node) \
	(node->i_ops->viop_unlink(node, name, unlink_node))
#define viop_lookup(parent, sub_dentry) \
	(parent->i_ops->viop_lookup(parent, sub_dentry))
#define viop_readdir(dir_vinode, dir, offset) \
	(dir_vinode->i_ops->viop_readdir(dir_vinode, dir, offset))
