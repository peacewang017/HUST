...
int map_pages(pagetable_t page_dir, uint64 va, uint64 size, uint64 pa, int perm)
{
	...
	for (first = ROUNDDOWN(va, PGSIZE),
	    last = ROUNDDOWN(va + size - 1, PGSIZE);
	     first <= last; first += PGSIZE, pa += PGSIZE) {
		if ((pte = page_walk(page_dir, first, 1)) == 0)
			return -1;
		if (*pte & PTE_V)
			panic("map_pages fails on mapping va (0x%lx) to pa (0x%lx)",
			      first, pa);
...
void kern_vm_init(void)
{
	...
	kern_vm_map(t_page_dir, KERN_BASE, DRAM_BASE,
		    (uint64)_etext - KERN_BASE,
...
void user_vm_unmap(pagetable_t page_dir, uint64 va, uint64 size, int free)
{
	...
	pte_t *target_pte = page_walk(page_dir, va, 0);
	if (target_pte == NULL) {
		return;
	} else {
		free_page((void *)PTE2PA(*target_pte));
		*target_pte &= (~PTE_V);
	}
}
