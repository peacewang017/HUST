...
int vfs_init()
{
	...
	ret = hash_table_init(&dentry_hash_table, dentry_hash_equal,
			      dentry_hash_func, NULL, NULL, NULL);
	if (ret != 0)
		return ret;

	ret = hash_table_init(&vinode_hash_table, vinode_hash_equal,
			      vinode_hash_func, NULL, NULL, NULL);
	if (ret != 0)
		return ret;
	return 0;
}
...
struct file *vfs_open(const char *path, int flags)
{
	...
	struct dentry *file_dentry =
		lookup_final_dentry(path, &parent, miss_name);
	...
	if (!file_dentry) {
		...
		if (creatable) {
			...
			struct vinode *new_inode =
				viop_create(parent->dentry_inode, file_dentry);
			if (!new_inode)
...
ssize_t vfs_read(struct file *file, char *buf, size_t count)
{
	...
	// actual reading.
	return viop_read(file->f_dentry->dentry_inode, buf, count,
			 &(file->offset));
}
...
ssize_t vfs_write(struct file *file, const char *buf, size_t count)
{
	...
	// actual writing.
	return viop_write(file->f_dentry->dentry_inode, buf, count,
			  &(file->offset));
}
...
int vfs_link(const char *oldpath, const char *newpath)
{
	...
	new_file_dentry = alloc_vfs_dentry(
		basename, old_file_dentry->dentry_inode, parent);
...
int vfs_unlink(const char *path)
{
	...
	int err =
		viop_unlink(parent->dentry_inode, file_dentry, unlinked_vinode);
	if (err)
		return -1;
...
int vfs_mkdir(const char *path)
{
	...
	struct vinode *new_dir_inode =
		viop_mkdir(parent->dentry_inode, new_dentry);
	if (!new_dir_inode) {
...
int vfs_closedir(struct file *file)
{
	...
	if (file->f_dentry->dentry_inode->i_ops->viop_hook_closedir) {
		if (file->f_dentry->dentry_inode->i_ops->viop_hook_closedir(
			    file->f_dentry->dentry_inode, file->f_dentry) !=
		    0) {
...
int vfs_rcwd(char *path)
{
	...
	char gap[] = "/";
	// read cwd
	struct dentry *this_dentry = current[hartid]->pfiles->cwd;

	// is root
	if (this_dentry == vfs_root_dentry) {
		str_insert_to_head(path, gap);
		return 0;
	}

	// not root
	while (1) {
		str_insert_to_head(path, this_dentry->name);
		this_dentry = this_dentry->parent;
		str_insert_to_head(path, gap);
		if (this_dentry == vfs_root_dentry) {
			break;
		}
...
int vfs_ccwd(char *path)
{
	...
	if (target_dentry == NULL) {
		sprint("vfs_ccwd: %s: no such file or directory\n", miss_name);
		return 1;
	} else if (target_dentry->dentry_inode->type !=
		   DIR_I) { // 检查目标dentry是否是文件，而非目录
		sprint("vfs_ccwd: %s: not a dentry\n", path);
	}
	current[hartid]->pfiles->cwd = target_dentry;
	...
	while (token != NULL) {
		if (strcmp(token, "..") == 0) {
			...
			if (this == NULL) {
				...
				// get subdir's vinode
				struct vinode *found_vinode = viop_lookup(
					(*parent)->dentry_inode, this);
				...
				struct vinode *same_inode = hash_get_vinode(
					found_vinode->sb, found_vinode->inum);
				if (same_inode != NULL) {
...
struct dentry *hash_get_dentry(struct dentry *parent, char *name)
{
	...
	return (struct dentry *)dentry_hash_table.virtual_hash_get(
		&dentry_hash_table, &key);
}
...
int hash_put_dentry(struct dentry *dentry)
{
	...
	int ret = dentry_hash_table.virtual_hash_put(&dentry_hash_table, key,
						     dentry);
	if (ret != 0)
		free_page(key);
	return ret;
...
{
	struct dentry_key key = { .parent = dentry->parent,
				  .name = dentry->name };
...
struct vinode *hash_get_vinode(struct super_block *sb, int inum)
{
	...
	return (struct vinode *)vinode_hash_table.virtual_hash_get(
		&vinode_hash_table, &key);
}
...
int hash_put_vinode(struct vinode *vinode)
{
	...
	int ret = vinode_hash_table.virtual_hash_put(&vinode_hash_table, key,
						     vinode);
	if (ret != 0)
		free_page(key);
	return ret;
